...
class WordleSolver:
    def __init__(self, word_list: List[str]):
        """Initialize with a list of valid 5-letter words."""
        self.word_list = [w.upper() for w in word_list if len(w) == 5]
        self.possible_words = set(self.word_list)
        
    def get_pattern(self, guess: str, answer: str) -> Tuple[int, ...]:
        """
        Get the pattern for a guess against an answer.
        0 = gray (not in word), 1 = yellow (in word, wrong position), 2 = green (correct position)
        """
        pattern = [0] * 5
    ...
    def filter_words(self, guess: str, pattern: Tuple[int, ...]) -> Set[str]:
    ...
    def calculate_entropy(self, guess: str, possible: Set[str]) -> float:
        """
    ...
    def get_best_guess(self, possible: Set[str], use_all_words: bool = True) -> str:
        """
        Find the guess that maximizes expected information gain.
        If use_all_words is True, consider all words; otherwise only possible answers.
        """
        if len(possible) <= 2:
            return list(possible)[0]
        
        # For first guess or large possibility space, consider all words
        candidates = self.word_list if use_all_words and len(possible) > 10 else possible
        
        best_guess = None
        best_entropy = -1
        
        for guess in candidates:
            entropy = self.calculate_entropy(guess, possible)
            
            # Tiebreaker: prefer words that are possible answers
            if entropy > best_entropy or (entropy == best_entropy and guess in possible):
                best_entropy = entropy
                best_guess = guess
        
        return best_guess
    
    def solve(self, answer: str, verbose: bool = True) -> int:
        """
        Solve for a specific answer and return number of guesses.
        """
        self.possible_words = set(self.word_list)
        guesses = 0
        ...
        while True:
            ...
            # Get best guess
            guess = self.get_best_guess(self.possible_words, use_all_words=(guesses == 1))
            pattern = self.get_pattern(guess, answer)
            
            if verbose:
                pattern_str = ''.join(['⬜' if p == 0 else '🟨' if p == 1 else '🟩' for p in pattern])
                print(f"Guess {guesses}: {guess} {pattern_str} ({len(self.possible_words)} possible)")
            ...
            self.possible_words = self.filter_words(guess, pattern)
    ...
    def interactive_solve(self):
        ...
        print("Enter pattern: 0=gray, 1=yellow, 2=green (e.g., 01220)")
        print()
        
        while True:
            # Suggest best guess
            guess = self.get_best_guess(self.possible_words, use_all_words=(guesses == 1))
            print(f"\nGuess {guesses + 1}: Try '{guess}' ({len(self.possible_words)} possibilities)")
            
            # Get user feedback
            pattern_str = input("Enter pattern (or 'q' to quit): ").strip()
            ...
            try:
                ...
                if len(pattern) != 5 or any(p not in [0, 1, 2] for p in pattern):
                    raise ValueError
            except:
                print("Invalid pattern. Use 5 digits (0, 1, or 2).")
                continue
            
            # Only increment guess count after valid pattern
            guesses += 1
            ...
            if not self.possible_words:
                print("No possible words remaining. Check your pattern!")
                break
            elif len(self.possible_words) <= 10:
                print(f"Remaining possibilities: {', '.join(sorted(self.possible_words))}")


# Example usage with complete Wordle answer list
if __name__ == "__main__":
    # Complete official Wordle answer list (2,315 words)
    wordle_answers = [
    ...
    # Initialize solver with complete answer list
    solver = WordleSolver(wordle_answers)
    
    # Demo: Solve a specific word
    print("=== Demo: Solving 'CRANE' ===")
    solver.solve("CRANE", verbose=True)
    
    print("\n" + "="*50 + "\n")
    
    # Interactive mode
    print("Starting interactive mode...")
    print("(You can test this by playing actual Wordle and entering the patterns)")
